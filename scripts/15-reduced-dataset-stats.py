...
def iterate_texts_in_subset(args, tokenizer, subset, subset_stats):
  ...
  iterate_reduced_event_files(
    reduced_dataset_dir(args.data_dir) / '07-sentence-level-alignment' / subset,
    extract_text_stats,
    filter_tsv
  )
...
def iterate_audio_files_in_subset(args, subset, subset_stats):
  ...
  iterate_reduced_event_files(
    reduced_dataset_dir(args.data_dir) / '08-audio-segments' / subset,
    extract_audio_stats,
    filter_wav
  )
...
def print_stats(subset, stats):
  ...
  print(f'Total duration: {sum(stats["audio_durations"])}')
